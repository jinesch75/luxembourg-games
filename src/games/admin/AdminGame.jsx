...
const CAT_COLORS = {
  housing:    { bg: '#FEF3C7', text: '#D97706', icon: '🏠' },
  work:       { bg: '#DBEAFE', text: '#1D4ED8', icon: '💼' },
  health:     { bg: '#D1FAE5', text: '#065F46', icon: '🏥' },
  transport:  { bg: '#E0E7FF', text: '#4338CA', icon: '🚌' },
  taxes:      { bg: '#FEE2E2', text: '#DC2626', icon: '📋' },
  family:     { bg: '#FCE7F3', text: '#BE185D', icon: '👨‍👩‍👧' },
  digital:    { bg: '#F3E8FF', text: '#7C3AED', icon: '🏛️' },
  finance:    { bg: '#CFFAFE', text: '#0E7490', icon: '💰' },
}

// ─── Level system (mirrors other games: 5 levels × 5 sub-levels = 125 scenarios) ──
...
// ─── Progress helpers (same logic as other games) ────────────────────────────
function parseProgress(progress) {
  const completed = progress?.completedSubLevels || {}
  for (let li = 0; li < ADMIN_LEVELS.length; li++) {
    const lvl = ADMIN_LEVELS[li]
    const done = completed[lvl.id] || 0
    if (done < lvl.subLevels) return { levelIdx: li, subLevel: done + 1 }
  }
  return { levelIdx: ADMIN_LEVELS.length - 1, subLevel: ADMIN_LEVELS.at(-1).subLevels, finished: true }
}

function totalSubLevelsDone(progress) {
  const completed = progress?.completedSubLevels || {}
  return ADMIN_LEVELS.reduce((acc, lvl) => acc + (completed[lvl.id] || 0), 0)
}

// ─── Shuffle options (Fisher-Yates, same as other games) ─────────────────────
...
function Intro({ t, progress, curLevel, curSubLevel, onStart }) {
  ...
  const totalSubs = ADMIN_LEVELS.reduce((acc, l) => acc + l.subLevels, 0)
    ...
    <div>
      ...
      <div className="container" style={{ paddingTop: 28 }}>
        ...
        >
          ...
          }}><ClipboardList size={26} strokeWidth={1.75} /></span>
...
function Done({ scores, t, progress, curLevel, curSubLevel, levelUpInfo, onReplay, onReset }) {
  ...
  const nextLevel = ADMIN_LEVELS[nextLevelIdx]
...
export default function AdminGame() {
  ...
  useEffect(() => {
    ...
    fetch('/api/content')
      .then(r => r.ok ? r.json() : {})
      .then(data => {
        if (!cancelled && data.admin?.length > 0) setSCENARIOS(data.admin)
      })
      .catch(() => {})
    return () => { cancelled = true }
  }, [])

  const [progress, setProgress] = useLocalStorage('letz-admin-progress-v1', {
  ...
  const curLevel = ADMIN_LEVELS[curLevelIdx]
  ...
  const handleNext = () => {
    ...
    if (currentIdx + 1 >= scenarios.length) {
      ...
      if (newLevelIdx > prevLevelIdx) setLevelUpInfo(ADMIN_LEVELS[newLevelIdx])

      setProgress(newProgressObj)
      setRoundScores(newScores)
      setDoneInfo({ level: curLevel, subLevel: curSubLevel })
      trackGameEvent('admin', 'complete', { score: sessionTotal, level: curLevel.id, subLevel: curSubLevel })
      setStep('done')
    } else {
      setRoundScores(newScores)
      setCurrentIdx(i => i + 1)
  ...
  if (step === 'intro') {
    return (
      <Intro
        ...
        onStart={() => { trackGameEvent('admin', 'start'); setStep('question') }}
      />
    )
  }
  ...
  const cat = CAT_COLORS[s.category] || CAT_COLORS.digital
    ...
    <div style={S.page}>
      <div style={S.container}>
        <div style={{ paddingTop: 16 }}>
          ...
          {/* Scenario card — replaces photo card */}
          <div style={{ ...S.card, marginTop: 12, marginBottom: 10, padding: '20px 20px' }}>
            ...
            <div style={{
              ...
              <span>{cat.icon}</span>
              <span>{s.category.charAt(0).toUpperCase() + s.category.slice(1)}</span>
            </div>
