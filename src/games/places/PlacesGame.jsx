...
// ─── Level system (mirrors quiz game: 5 levels × 5 sub-levels = 125 places) ──
...
function parseProgress(progress) {
  ...
  for (let li = 0; li < PLACES_LEVELS.length; li++) {
    const lvl = PLACES_LEVELS[li]
    const done = completed[lvl.id] || 0
    if (done < lvl.subLevels) return { levelIdx: li, subLevel: done + 1 }
  }
  return { levelIdx: PLACES_LEVELS.length - 1, subLevel: PLACES_LEVELS.at(-1).subLevels, finished: true }
}

function totalSubLevelsDone(progress) {
  const completed = progress?.completedSubLevels || {}
  return PLACES_LEVELS.reduce((acc, lvl) => acc + (completed[lvl.id] || 0), 0)
}

// ─── Shuffle options (Fisher-Yates, same as quiz game) ───────────────────────
function shufflePlaceOptions(p) {
...
// ─── Photo mapping cache (fetched once from /api/place-photos) ───────────────
...
function getPhotoMap() {
  ...
  if (!_photoMapPromise) {
    _photoMapPromise = fetch('/api/place-photos')
      .then(r => r.ok ? r.json() : {})
...
function WikiImage({ placeId, wikiTitle, size = 220, noPhoto = false, photoPos = null, imageUrl = null }) {
  ...
  useEffect(() => {
    ...
    getPhotoMap().then(map => {
      if (cancelled) return
      const entry = map[placeId]
      if (entry?.file) {
        setSrc(`/place-photos/${entry.file}`)
        return
      }
  ...
  }, [placeId, wikiTitle, noPhoto, imageUrl])

  // Default object-position: center center for buildings
  // Custom photoPos can override for specific photos
  const objPosition = photoPos || 'center center'
...
function Intro({ t, progress, curLevel, curSubLevel, onStart }) {
  ...
  const totalSubs = PLACES_LEVELS.reduce((acc, l) => acc + l.subLevels, 0)
    ...
    <div>
      ...
      <div className="container" style={{ paddingTop: 28 }}>
        ...
        >
          ...
          }}><Building size={26} strokeWidth={1.75} /></span>
...
function Done({ scores, places, t, progress, curLevel, curSubLevel, levelUpInfo, onReplay, onReset }) {
  ...
  const nextLevel = PLACES_LEVELS[nextLevelIdx]
...
export default function PlacesGame() {
  ...
  useEffect(() => {
    ...
    fetch('/api/content')
      .then(r => r.ok ? r.json() : {})
      .then(data => {
        if (!cancelled && data.places?.length > 0) setPLACES(data.places)
      })
      .catch(() => {})
    return () => { cancelled = true }
  }, [])

  const [progress, setProgress] = useLocalStorage('letz-places-progress-v2', {
  ...
  const curLevel = PLACES_LEVELS[curLevelIdx]

  // Get 5 places for this sub-level
  const places = useMemo(
    () => getSubLevelPlaces(curLevel.id, curSubLevel, PLACES),
    [curLevel.id, curSubLevel, PLACES],
  )

  // Shuffle options so correct answer isn't always A
  const shuffledPlaces = useMemo(
    () => places.map(shufflePlaceOptions),
    [places],
  )
  ...
  const handleNext = () => {
    ...
    const p = shuffledPlaces[currentIdx]
    ...
    if (currentIdx + 1 >= places.length) {
      ...
      if (newLevelIdx > prevLevelIdx) setLevelUpInfo(PLACES_LEVELS[newLevelIdx])

      setProgress(newProgressObj)
      setRoundScores(newScores)
      setDoneInfo({ level: curLevel, subLevel: curSubLevel })
      trackGameEvent('places', 'complete', { score: sessionTotal, level: curLevel.id, subLevel: curSubLevel })
      setStep('done')
    } else {
      setRoundScores(newScores)
      setCurrentIdx(i => i + 1)
  ...
  if (step === 'intro') {
    return (
      <Intro
        ...
        onStart={() => { trackGameEvent('places', 'start'); setStep('question') }}
      />
    )
  }
      ...
      <div style={S.page}>
        <div style={S.container}>
          <div style={{ paddingTop: 40, textAlign: 'center' }}>
            <p style={{ color: '#94A3B8', marginBottom: 16 }}>Could not load places for this level.</p>
    ...
    <div style={S.page}>
      <div style={S.container}>
        <div style={{ paddingTop: 16 }}>
          ...
          <ProgressBar current={currentIdx + 1} total={places.length} color={curLevel.color} />

          {/* Photo card — compact for mobile */}
          <div style={{ ...S.card, marginTop: 12, marginBottom: 10, textAlign: 'center', padding: '16px 20px' }}>
            <WikiImage key={p.id || currentIdx} placeId={p.id} wikiTitle={p.wikiTitle} size={180} noPhoto={p.noPhoto} photoPos={p.photoPos} imageUrl={p.imageUrl} />
                ...
                >
                  ...
                  <span style={{ flex: 1, textAlign: 'left', lineHeight: 1.3 }}>
                    <span style={{ display: 'block', fontSize: '0.95rem', fontWeight: 700 }}>
                      {currentIdx + 1 >= places.length ? 'See Results' : 'Next Place'}
                    </span>
                    <span style={{ display: 'block', fontSize: '0.7rem', opacity: 0.65, fontWeight: 400, marginTop: 1 }}>
                      {currentIdx + 1 >= places.length
                        ? 'See your results'
                        : `Place ${currentIdx + 2} of ${places.length}`}
                    </span>
                  </span>
                  <span style={{ opacity: 0.55, fontSize: '1.1rem' }}>→</span>
                </button>
