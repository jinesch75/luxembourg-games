/**
 * AdminFamous — view and edit all famous-people entries.
...
// ── Person card (view mode) ───────────────────────────────────────────────
function PersonCard({ p, onEdit }) {
...
// ── Person editor ─────────────────────────────────────────────────────────
function PersonEditor({ p, onSave, onCancel }) {
        ...
        <div style={{ flex: 1 }}>
          ...
          <div>
            ...
            <input
              ...
              style={{ ...inputStyle(), height: 36 }}
              placeholder="e.g. Henri,_Grand_Duke_of_Luxembourg"
            />
          </div>
        </div>
      </div>
...
// ── Main component ────────────────────────────────────────────────────────
export default function AdminFamous() {
  ...
  const saveToServer = async (updatedPeople) => {
    ...
    try {
      ...
      const res = await fetch('/api/content', {
        ...
        body: JSON.stringify({ ...current, people: updatedPeople }),
      })
  ...
  const handleSave = (updatedP) => {
    const updated = people.map(p => p.id === updatedP.id ? updatedP : p)
    setPeople(updated)
    setEditingId(null)
  ...
  const resetToDefaults = async () => {
    if (!confirm('Reset ALL famous people to the original defaults? This cannot be undone.')) return
    const r = await fetch('/api/content').then(r => r.json()).catch(() => ({}))
    const { people: _, ...rest } = r
  ...
  return (
    <div>
      ...
      {filtered.map(p => (
        editingId === p.id
          ? <PersonEditor key={p.id} p={p} onSave={handleSave} onCancel={() => setEditingId(null)} />
          : <PersonCard key={p.id} p={p} onEdit={() => setEditingId(p.id)} />
      ))}
